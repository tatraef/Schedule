...
namespace Schedule.ViewModels
{
    class TimelineViewModel
    {
        ...
        public void LoadRaitingForStudent(List<Couple> couples, DateTime NeedDate)
        {
            ...
            //Определение, когда было загружено расписание рейтингов
            DateTime lastUpdate = DateTime.Parse((string) App.Current.Properties["updateRait"]);
            TimeSpan difference = NeedDate.Subtract(lastUpdate);
            int days = difference.Days;
            //если более чем 30 дней назад, то значит нового расписания еще нет,
            //показать в таймлайне просто "Рейтинги"
            if (Math.Abs(days) > 30)
            {
                NothingInteresting(couples, "Р");
            }
            else
            {
